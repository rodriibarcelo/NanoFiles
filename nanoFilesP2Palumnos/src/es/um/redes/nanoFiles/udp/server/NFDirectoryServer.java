package es.um.redes.nanoFiles.udp.server;

import java.io.IOException;
import java.net.DatagramPacket;
import java.net.DatagramSocket;
import java.net.InetSocketAddress;
import java.net.SocketException;
import java.util.ArrayList;
import java.util.HashMap;
import java.util.LinkedList;
import java.util.Random;

import es.um.redes.nanoFiles.application.NanoFiles;
import es.um.redes.nanoFiles.logic.NFControllerLogicDir;
import es.um.redes.nanoFiles.udp.message.DirMessage;
import es.um.redes.nanoFiles.udp.message.DirMessageOps;
import es.um.redes.nanoFiles.util.FileInfo;

public class NFDirectoryServer {
	/**
	 * Número de puerto UDP en el que escucha el directorio
	 */
	public static final int DIRECTORY_PORT = 6868;

	/**
	 * Socket de comunicación UDP con el cliente UDP (DirectoryConnector)
	 */
	private DatagramSocket socket = null;
	/**
	 * Estructura para guardar los nicks de usuarios registrados, y clave de sesión
	 * 
	 */
	private HashMap<String, Integer> nicks;
	/**
	 * Estructura para guardar las claves de sesión y sus nicks de usuario asociados
	 * 
	 */
	private HashMap<Integer, String> sessionKeys;
	/*
	 * TODO: Añadir aquí como atributos las estructuras de datos que sean necesarias
	 * para mantener en el directorio cualquier información necesaria para la
	 * funcionalidad del sistema nanoFilesP2P: ficheros publicados, servidores
	 * registrados, etc.
	 */
	private HashMap<String, Integer> fileServer;
	private HashMap<String, String> ip;
	private ArrayList<String> nameHashTam;


	/**
	 * Generador de claves de sesión aleatorias (sessionKeys)
	 */
	Random random = new Random();
	/**
	 * Probabilidad de descartar un mensaje recibido en el directorio (para simular
	 * enlace no confiable y testear el código de retransmisión)
	 */
	private double messageDiscardProbability;

	public NFDirectoryServer(double corruptionProbability) throws SocketException {
		/*
		 * Guardar la probabilidad de pérdida de datagramas (simular enlace no
		 * confiable)
		 */
		messageDiscardProbability = corruptionProbability;
		/*
		 * TODO: (Boletín UDP) Inicializar el atributo socket: Crear un socket UDP
		 * ligado al puerto especificado por el argumento directoryPort en la máquina
		 * local,
		 */
		socket = new DatagramSocket(DIRECTORY_PORT);
		/*
		 * TODO: (Boletín UDP) Inicializar el resto de atributos de esta clase
		 * (estructuras de datos que mantiene el servidor: nicks, sessionKeys, etc.)
		 */
		nicks = new HashMap<String, Integer>();
		
		sessionKeys =  new HashMap<Integer, String>();
		fileServer = new HashMap<String, Integer>();
		ip = new HashMap<String, String>();
		nameHashTam = new ArrayList<String>();
		if (NanoFiles.testMode) {
			if (socket == null || nicks == null || sessionKeys == null) {
				System.err.println("[testMode] NFDirectoryServer: code not yet fully functional.\n"
						+ "Check that all TODOs in its constructor and 'run' methods have been correctly addressed!");
				System.exit(-1);
			}
		}
	}

	public void run()  {
		byte[] receptionBuffer = null;
		InetSocketAddress clientAddr = null;
		int dataLength = -1;
		/*
		 *  (Boletín UDP) Crear un búfer para recibir datagramas y un datagrama
		 * asociado al búfer
		 */
		receptionBuffer = new byte[DirMessage.PACKET_MAX_SIZE];
		DatagramPacket paqueteReceptor = new DatagramPacket(receptionBuffer, receptionBuffer.length);



		System.out.println("Directory starting...");

		while (true) { // Bucle principal del servidor de directorio

			//  (Boletín UDP) Recibimos a través del socket un datagrama
			try {
				socket.receive(paqueteReceptor);
			} catch (IOException e) {
				System.err.println("Error al recibir el paquete");
			}

			//  (Boletín UDP) Establecemos dataLength con longitud del datagrama
			// recibido
			dataLength = paqueteReceptor.getLength();
			//  (Boletín UDP) Establecemos 'clientAddr' con la dirección del cliente,
			// obtenida del
			// datagrama recibido
			clientAddr = (InetSocketAddress) paqueteReceptor.getSocketAddress();

			if (NanoFiles.testMode) {
				if (receptionBuffer == null || clientAddr == null || dataLength < 0) {
					System.err.println("NFDirectoryServer.run: code not yet fully functional.\n"
							+ "Check that all TODOs have been correctly addressed!");
					System.exit(-1);
				}
			}
			System.out.println("Directory received datagram from " + clientAddr + " of size " + dataLength + " bytes");

			// Analizamos la solicitud y la procesamos
			if (dataLength > 0) {
				String messageFromClient = null;
				/*
				 *  (Boletín UDP) Construir una cadena a partir de los datos recibidos en
				 * el buffer de recepción
				 */
				messageFromClient = new String(receptionBuffer, 0, paqueteReceptor.getLength());

				if (NanoFiles.testMode) { // En modo de prueba (mensajes en "crudo", boletín UDP)
					System.out.println("[testMode] Contents interpreted as " + dataLength + "-byte String: \""
							+ messageFromClient + "\"");
					double rand = Math.random();
					if (rand < messageDiscardProbability) {
						System.err.println("Directory DISCARDED datagram from " + clientAddr);
						continue;
					}
					/*
					 *  (Boletín UDP) Comprobar que se ha recibido un datagrama con la cadena
					 * "login" y en ese caso enviar como respuesta un mensaje al cliente con la
					 * cadena "loginok". Si el mensaje recibido no es "login", se informa del error
					 * y no se envía ninguna respuesta.
					 */
					if(messageFromClient.equals("login")) {
						
						int sessionKey = new Random().nextInt(10000);
						while(sessionKeys.containsKey(sessionKey)) {
							sessionKey = new Random().nextInt(10000);
						}
						 
						String logok = "loginok&"+sessionKey;
						
						byte cadenaRes[] = logok.getBytes();
						
						DatagramPacket respuesta = new DatagramPacket(cadenaRes, cadenaRes.length, clientAddr);
						try {
							socket.send(respuesta);
						} catch (IOException e) {
							System.err.println("Error al enviar el datagrama");
						}
					}else {
						System.err.println("Error, login no realizado");
					}
					

				} else { // Servidor funcionando en modo producción (mensajes bien formados)

					// Vemos si el mensaje debe ser ignorado por la probabilidad de descarte
					double rand = Math.random();
					if (rand < messageDiscardProbability) {
						System.err.println("Directory DISCARDED datagram from " + clientAddr);
						continue;
					}
					//System.out.println("Message from client: "+messageFromClient);
					//String[] logYnick = messageFromClient.split("&");
					DirMessage mensajeRecibido = DirMessage.fromString(messageFromClient);
					DirMessage mensajeRespuesta = buildResponseFromRequest(mensajeRecibido, clientAddr);
					String	mensajeConvertido = mensajeRespuesta.toString();
					DatagramPacket respuesta;
					if(mensajeRespuesta.getSessionKey()!=-1) {
						byte cadenaRes[] = mensajeConvertido.getBytes();
						respuesta = new DatagramPacket(cadenaRes, cadenaRes.length, clientAddr);
						try {
							socket.send(respuesta);
						} catch (IOException e) {
							System.err.println("Error al enviar el datagrama");
						}
					}else {
						byte cadenaRes[] = mensajeConvertido.getBytes();
						respuesta = new DatagramPacket(cadenaRes, cadenaRes.length, clientAddr);
						try {
							socket.send(respuesta);
						} catch (IOException e) {
							System.err.println("Error al enviar el datagrama");
						}
					}
					/*
					 * TODO: Construir String partir de los datos recibidos en el datagrama. A
					 * continuación, imprimir por pantalla dicha cadena a modo de depuración.
					 * Después, usar la cadena para construir un objeto DirMessage que contenga en
					 * sus atributos los valores del mensaje (fromString).
					 */
					/*
					 * TODO: Llamar a buildResponseFromRequest para construir, a partir del objeto
					 * DirMessage con los valores del mensaje de petición recibido, un nuevo objeto
					 * DirMessage con el mensaje de respuesta a enviar. Los atributos del objeto
					 * DirMessage de respuesta deben haber sido establecidos con los valores
					 * adecuados para los diferentes campos del mensaje (operation, etc.)
					 */
					/*
					 * TODO: Convertir en string el objeto DirMessage con el mensaje de respuesta a
					 * enviar, extraer los bytes en que se codifica el string (getBytes), y
					 * finalmente enviarlos en un datagrama
					 */



				}
			} else {
				System.err.println("Directory ignores EMPTY datagram from " + clientAddr);
			}

		}
	}

	private DirMessage buildResponseFromRequest(DirMessage msg, InetSocketAddress clientAddr) {
		/*
		 * TODO: Construir un DirMessage con la respuesta en función del tipo de mensaje
		 * recibido, leyendo/modificando según sea necesario los atributos de esta clase
		 * (el "estado" guardado en el directorio: nicks, sessionKeys, servers,
		 * files...)
		 */
		String operation = msg.getOperation();
		System.out.println("Mensaje recibido: ");
		System.out.println(msg.toString());
		DirMessage response = null;


		switch (operation) {
		case DirMessageOps.OPERATION_LOGIN: {
			String username = msg.getNickname();
			/*
			 * Comprobamos si tenemos dicho usuario registrado (atributo "nicks"). Si
			 * no está, generamos su sessionKey (número aleatorio entre 0 y 1000) y añadimos
			 * el nick y su sessionKey asociada. NOTA: Puedes usar random.nextInt(10000)
			 * para generar la session key
			 */
			boolean contieneNick = nicks.containsKey(username);
			if(!contieneNick) {
				int sessionKey = random.nextInt(10000);
				while(sessionKeys.containsKey(sessionKey))
					sessionKey = random.nextInt(10000);
				sessionKeys.put(sessionKey, username);
				nicks.put(username, sessionKey);
				ip.put(username, clientAddr.getAddress().getHostAddress());
			}
			/*
			 * Construimos un mensaje de respuesta que indique el éxito/fracaso del
			 * login y contenga la sessionKey en caso de éxito, y lo devolvemos como
			 * resultado del método.
			 */
			if(!contieneNick) {
				response= new DirMessage(DirMessageOps.OPERATION_LOGIN_OK, nicks.get(username));
				System.out.println("Login exitoso.");
			}else {
				response= new DirMessage(DirMessageOps.OPERATION_LOGIN_OK, -1);
				System.err.println("Login ha fracasado, el nickname esta en uso");
			}
			/*
			 *  Imprimimos por pantalla el resultado de procesar la petición recibida
			 * (éxito o fracaso) con los datos relevantes, a modo de depuración en el
			 * servidor
			 */

			break;
		}case DirMessageOps.OPERATION_GET_USER_LIST:{
			System.out.println("Obtener la lista de usuarios.");
			if(nicks.size()>0) {
				String[] users = new String[nicks.size()];
				int i=0;
				for(String nom : nicks.keySet()) {
					for(String nickn : fileServer.keySet()) {
						System.out.println("Servidor " + nickn);
					}
					if(fileServer.containsKey(nom)) {
						users[i] = new String(nom+"-true");
					}else {
						users[i] = new String(nom+"-false");
					}
					i++;
				}
				response = new DirMessage(DirMessageOps.OPERATION_SEND_USER_LIST, users);
			}else {
				System.err.println("Lista de usuarios vacía");
				response = new DirMessage(DirMessageOps.OPERATION_SEND_USER_LIST, "");
			}
				
			break;
		}
		case DirMessageOps.OPERATION_LOGOUT:
		{
			System.out.println("Hacer logout");
			
			if(sessionKeys.containsKey(msg.getSessionKey())) {
				 String nick = sessionKeys.remove(msg.getSessionKey());
				 nicks.remove(nick);
				 response = new DirMessage(DirMessageOps.OPERATION_LOGOUT_OK, true);
			}else {
				System.err.println("Logout no realizado, no se han encontrado nick y sessionKey");
				response = new DirMessage(DirMessageOps.OPERATION_LOGOUT_OK, false);
			}
			break;
		}
		case DirMessageOps.OPERATION_REG_SERVER_PORT:
		{
			System.out.println("Guardar puerto de escucha");
			String nickname = sessionKeys.get(msg.getSessionKey());
			if(sessionKeys.containsKey(msg.getSessionKey()) && !fileServer.containsKey(nickname)) {
				fileServer.put(nickname, msg.getPort());
				response = new DirMessage(DirMessageOps.OPERATION_REG_SERVER_PORT_OK, true);
			}else {
				System.err.println("Nick ya es servidor");
				response = new DirMessage(DirMessageOps.OPERATION_REG_SERVER_PORT_OK, false);
			}
			break;
		}
		case DirMessageOps.OPERATION_PUBLISH:
		{
			System.out.println("Publicar ficheros peers");
			if(sessionKeys.containsKey(msg.getSessionKey())) {
				int i = 0;
				String[] hash =  msg.getFilehash();
				String[] tam = msg.getFilesize();				
				for(String str : msg.getFilename()) {
					nameHashTam.add(str +"-"+hash[i]+"-"+tam[i]);
					i++;
				}
				response = new DirMessage(DirMessageOps.OPERATION_PUBLISH_OK, true);
			}else {
				response = new DirMessage(DirMessageOps.OPERATION_PUBLISH_OK, false);
			}
			break;
		}
		case DirMessageOps.OPERATION_UNREGISTER_SERVER:
		{
			if(sessionKeys.containsKey(msg.getSessionKey())) {
				String nick = sessionKeys.get(msg.getSessionKey());
				
				if(fileServer.containsKey(nick)) {
					fileServer.remove(nick);
					response = new DirMessage(DirMessageOps.OPERATION_UNREGISTER_SERVER_OK, true);
				}
			}else {
				response = new DirMessage(DirMessageOps.OPERATION_UNREGISTER_SERVER_OK, false);
			}
			break;
		}
		case DirMessageOps.OPERATION_SEND_LOOKUP_ADDR:
		{
			if(sessionKeys.containsKey(msg.getSessionKey())) {
				String nick = msg.getNickname();
				String addr = ip.get(nick);
				String port = Integer.toString(fileServer.get(nick));
				String resp = new String(addr+"-"+port);
				response = new DirMessage(DirMessageOps.OPERATION_RCV_LOOKUP_ADDR, resp);
			}else {
				response = new DirMessage(DirMessageOps.OPERATION_RCV_LOOKUP_ADDR, "");
			}
			break;
		}
		case DirMessageOps.OPERATION_FILELIST:
		{
			if(sessionKeys.containsKey(msg.getSessionKey())) {
				String[] fname = new String[nameHashTam.size()];
				String[] fhash = new String[nameHashTam.size()];
				String[] fsize = new String[nameHashTam.size()];
				int i = 0;
				for(String nam : nameHashTam) {
					String[] sep = nam.split("-");
					fname[i] = sep[0];
					fhash[i] = sep[1];
					fsize[i] = sep[2];
				//System.out.println("NAME: " + fhash[i] + " " + fname[i] +" " + fsize[i]);
					i++;
				}
				response = new DirMessage(DirMessageOps.OPERATION_FILELIST_OK, fname, fhash, fsize);
			}else {
				response = new DirMessage(DirMessageOps.OPERATION_FILELIST_OK, null, null, null);
			}
			break;
		}
		default:
			System.out.println("Unexpected message operation: \"" + operation + "\"");
		}
		return response;

	}
}
