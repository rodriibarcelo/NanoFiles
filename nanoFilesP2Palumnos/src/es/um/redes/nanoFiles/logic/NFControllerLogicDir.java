...
public class NFControllerLogicDir {
	...
	public boolean doLogout() {
		...
		if(result) 
			...
			System.err.println("Logout ha fracasado.w");
		return result;
	}
	...
	protected boolean getAndPrintUserList() {
		...
		if(users == null)
			System.err.println("No se ha podido obtener la lista de usuarios");
		else {
			System.out.println("Obteniendo lista de usuarios...");
			int i = 0;
			for(String us : users) {
				String[] usuServ = us.split("-");
				String usuario = usuServ[0];
				String esServ = usuServ[1];
				System.out.println("Usuario " + i +": " + usuario );
				System.out.println("Es servidor: " + esServ);
				i++;
			}
			result = true;
		}
	...
	protected boolean getAndPrintFileList() {
		 ...
		 */
		boolean result = false;
		FileInfo[] finfo = directoryConnector.getFileList();
		System.out.println("Archivos servidos:");
		if(finfo!=null) {
			for(int i=0;i<finfo.length;i++) {
				System.out.println(finfo[i].fileName + " " + finfo[i].fileHash + " " + finfo[i].fileSize);
			}
			result = true;
		}else {
			result = false;
	...
	public InetSocketAddress getServerAddress(String serverNicknameOrSocketAddr) {
		...
		if (serverNicknameOrSocketAddr.contains(":")) { // Then it has to be a socket address (IP:port)
			 ...
			 */
			String[] ipPort = serverNicknameOrSocketAddr.split(":");
			String ip = ipPort[0];
			String port = ipPort[1];
			int puerto = Integer.parseInt(port);
			try {
				fserverAddr = new InetSocketAddress(InetAddress.getByName(ip), puerto);
	...
	public boolean unregisterFileServer() {
		 ...
		 */
		boolean result = false;
		result = directoryConnector.unregisterFileServer();
		return result;
	}
