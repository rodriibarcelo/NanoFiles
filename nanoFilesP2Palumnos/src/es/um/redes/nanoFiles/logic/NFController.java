...
public class NFController {
	...
	private static final byte SERVE_FILES = 2;
	/*
	...
	public void processCommand() {
		...
		case NFCommands.COM_FGSERVE:
			...
			int puerto = PORT;
			while(!registrado) {
				registrado = controllerDir.registerFileServer(puerto+i);
			}
			controllerPeer.foregroundServeFiles();
			controllerDir.unregisterFileServer();
			
			//controllerDir.registerFileServer(controllerPeer.getFGPort());
			break;
	...
	public boolean canProcessCommandInCurrentState() {
		...
		case NFCommands.COM_BGSERVE:
		{
			if(currentState != LOGGED_IN) {
				commandAllowed = false;
				System.err.println("* You cannot bgserve because you are not logged in the directory");
			}
			break;
		}
		case NFCommands.COM_DOWNLOADFROM:
		{
			if(currentState != LOGGED_IN && currentState != SERVE_FILES ) {
				commandAllowed = false;
				System.err.println("* You cannot downloadfrom because you are not logged in the directory");
			}
			break;
		}
		case NFCommands.COM_FGSERVE:
		{
			if(currentState != LOGGED_IN) {
				commandAllowed = false;
				System.err.println("* You cannot fgserve because you are not logged in the directory or you are already serving files");
			}
			break;
		}
		case NFCommands.COM_FILELIST:
		{
			if(currentState != LOGGED_IN && currentState != SERVE_FILES) {
				commandAllowed = false;
				System.err.println("* You cannot filelist because you are not logged in the directory");
			}
			break;
		}
		case NFCommands.COM_QUIT:
		...
		case NFCommands.COM_PUBLISH:
		{
			if(currentState != LOGGED_IN && currentState != SERVE_FILES) {
				commandAllowed = false;
				System.err.println("* You cannot publish because you are not logged in the directory");
			}
			break;
		}
		...
		case NFCommands.COM_LOGOUT:
		{
			if(currentState != LOGGED_IN) {
				commandAllowed = false;
				System.err.println("* You cannot logout because you are not logged in the directory or you are still serving files");
			}
			break;
		}
		case NFCommands.COM_USERLIST:
		{
			if(currentState != LOGGED_IN && currentState != SERVE_FILES) {
				commandAllowed = false;
				System.err.println("* You cannot userlist because you are not logged in the directory");
			}
			break;
		}
